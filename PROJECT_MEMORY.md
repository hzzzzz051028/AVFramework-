...
### 2026-09-01 — 修复 AirPlay 与待机页 KMS 竞争，扩展待机信息

- 根因：AirPlay 与 `screencast-standby.service` 同时 active，两个 GStreamer 进程都使用 `kmssink plane-id=71`。单靠 AirPlay 的 `ExecStartPre=systemctl stop` 不足以阻止其他路径再次启动 `Restart=always` 的待机服务，导致叠画、闪烁和 AirPlay 不稳定。
